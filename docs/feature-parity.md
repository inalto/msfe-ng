...
| Full message content in the UI (MailControl "Contents") | MailScanner `Archive Mail` → generated `archive.rules` (per-domain opt-out), body path recorded per message (`maillog.body_path`), retention via `bodydays` over both trees | M10 |
